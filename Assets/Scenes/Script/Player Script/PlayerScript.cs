...
public class PlayerScript : MonoBehaviour{
    ...
    void Awake(){
        ...
        boostMain = boost.main;  //  SERA´QUE ISSO É O DEFEITO?
     ...
     void FixedUpdate(){
        if(Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.A)){
            if (!LeanTween.isTweening(gameObject)){
                ...
                if(dir != Direction.LEFT){
                    LeanTween.rotateAroundLocal(gameObject, Vector3.up, 180, 0.3f).setOnComplete(TurnLeft);
                }else{
            ...
            if (!LeanTween.isTweening(gameObject)){
                ...
                {
                    LeanTween.rotateAroundLocal(gameObject, Vector3.up, -180, 0.3f).setOnComplete(TurnRight);
                }
                else
                {
                    transform.Translate(Vector3.forward * speed * Time.deltaTime);
        ...
        if (Input.GetKey(KeyCode.Z)){
            ...
            constForce.force = new Vector3(0f, -18f, 0f);
         ...
         if (!LeanTween.isTweening(gameObject)){
            ...
            for(int i= 0; i < 5; i++){
                Vector3 origin = pos + Vector3.up * Random.Range(-1f, 1f) +
                         Vector3.left * Random.Range(-1, 1f);
                ...
                temp.SendMessage("LaunchMissile", tarPos);  // errasse?

            } // tarPos = traget position
        }
